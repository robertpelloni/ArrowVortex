...
T OnsetDetectionFunction<T>::energyDifference (const std::vector<T>& buffer)
{
    ...
    difference = sum - prevEnergySum; // sample is first order difference in ene
rgy
...
//===========================================================
template <class T>
T OnsetDetectionFunction<T>::spectralDifference (const std::vector<T>& magnitude
Spectrum)
{
    ...
    for (size_t i = 0; i < magnitudeSpectrum.size(); i++)
    {
        ...
        T diff = magnitudeSpectrum[i] - prevMagnitudeSpectrum_spectralDifference
[i];
...
//===========================================================
template <class T>
T OnsetDetectionFunction<T>::spectralDifferenceHWR (const std::vector<T>& magnit
udeSpectrum)
{
    T sum = 0; // initialise sum to zero
...
//===========================================================
template <class T>
T OnsetDetectionFunction<T>::complexSpectralDifference (const std::vector<T>& ff
tReal, const std::vector<T>& fftImag)
{
    T dev, pdev;
    T sum;
    T magDiff, phaseDiff;
...
prevPhaseSpectrum2_complexSpectralDifference[i];
        ...
        // calculate magnitude difference (real part of Euclidean distance betwe
en complex frames)
        magDiff = magVal - prevMagnitudeSpectrum_complexSpectralDifference[i];

        // calculate phase difference (imaginary part of Euclidean distance betw
een complex frames)
        ...
        prevPhaseSpectrum2_complexSpectralDifference[i] = prevPhaseSpectrum_comp
lexSpectralDifference[i];
...
//===========================================================
template <class T>
T OnsetDetectionFunction<T>::highFrequencyContent (const std::vector<T>& magnitu
deSpectrum)
{
    T sum = 0;
