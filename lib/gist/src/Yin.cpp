...
T Yin<T>::pitchYin (const std::vector<T>& frame)
{
    ...
    // check that we can interpolate (i.e. that period isn't first or last eleme
nt)
    if ((period > 0) && (period < (delta.size() - 1)))
    {
        // parabolic interpolation
        fPeriod = parabolicInterpolation (period,delta[period-1],delta[period],d
elta[period+1]);
    }
...
//===========================================================
template <class T>
void Yin<T>::cumulativeMeanNormalisedDifferenceFunction (const std::vector<T>& f
rame)
{
    T cumulativeSum = 0.0;
    ...
    for (unsigned long i = minPeriod; i < (delta.size() - 1); i++)
    {
        ...
        if (delta[i] < thresh)
        {
            if ((delta[i] < delta[i-1]) && (delta[i] < delta[i+1]))
            {
                ...
                // look for them in order, this is the first one, so we accept i
t
                // as the candidate period (i.e. the minimum period), and break
the loop
                period = i;
