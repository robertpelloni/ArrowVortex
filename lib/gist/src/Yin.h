//=======================================================================
/** @file Yin.h
 *  @brief Implementation of the YIN pitch detection algorithm (de Cheveigné and
 Kawahara,2002)
 *  @author Adam Stark
...
private:
     ...
     * @param y1 the value of the cumulative mean normalised difference function
 at (period-1)
     * @param y2 the value of the cumulative mean normalised difference function
 at (period)
     * @param y3 the value of the cumulative mean normalised difference function
 at (period+1)
     * @returns the interpolated period
     */
    T parabolicInterpolation (unsigned long period,T y1,T y2,T y3);

    /** calculates the period candidate from the cumulative mean normalised diff
erence function
     * @param delta the cumulative mean normalised difference function
    ...
    /** the previous period estimate found by the algorithm last time it was cal
led - initially set to 1.0 */
    T prevPeriodEstimate;

    /** the sampling frequency */
    int fs;

    /** the minimum period the algorithm will look for. this is set indirectly b
y setMaxFrequency() */
    int minPeriod;
