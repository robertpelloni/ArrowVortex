...
public:
    ...
    /** Set the frame size - N.B. this will be twice the length of the magnitude
 spectrum passed to calculateMFCC()
    ...
    //=======================================================================
    /** Calculates the Mel Frequency Cepstral Coefficients from the magnitude sp
ectrum of a signal. The result
     * is stored in the public vector MFCCs.
     *
     * Note that the magnitude spectrum passed to the function is not the full m
irrored magnitude spectrum, but
     * only the first half. The frame size passed to the constructor should be t
wice the length of the magnitude spectrum.
     * @param magnitudeSpectrum the magnitude spectrum in vector format
     */
    void calculateMelFrequencyCepstralCoefficients (const std::vector<T>& magnit
udeSpectrum);

    /** Calculates the magnitude spectrum on a Mel scale. The result is stored i
n
...
private:
    /** Initialises the parts of the algorithm dependent on frame size, sampling
 frequency
 ...
 performing it in place
     ...
     */
    void discreteCosineTransform (std::vector<T>& inputSignal, const std::size_t
 numElements);

    /** Calculates the triangular filters used in the algorithm. These will be d
ifferent depending
     * upon the frame size, sampling frequency and number of coefficients and so
