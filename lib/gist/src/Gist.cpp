//=======================================================================
/** @file Gist.cpp
 *  @brief Implementation for all relevant parts of the 'Gist' audio analysis li
brary
 *  @author Adam Stark
...
void Gist<T>::setAudioFrameSize (int audioFrameSize)
{
    ...
    windowFunction = WindowFunctions<T>::createWindow (audioFrameSize, windowTyp
e);
...
{
. I will re-run the pre-commit steps to ensure the new changes are correct.    return coreFrequencyDomainFeatures.spectralFlatness (magnitudeSpectrum);
}
...
void Gist<T>::performFFT()
{
    ...
    {
        magnitudeSpectrum[i] = sqrt ((fftReal[i] * fftReal[i]) + (fftImag[i] * f
ftImag[i]));
    }
}
