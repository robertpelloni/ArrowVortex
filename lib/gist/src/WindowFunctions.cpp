...
//===========================================================
template <class T>
std::vector<T> WindowFunctions<T>::createWindow (int numSamples, WindowType wind
owType)
{
    ...
    else // NOTE THIS COVERS THE RECTANGULAR WINDOW CASE AND ANY OTHER UNKNOWN T
YPE
    {
...
std::vector<T> WindowFunctions<T>::createHanningWindow (int numSamples)
{
    ...
    T numSamplesMinus1 = (T) (numSamples - 1);          // the number of samples
 minus 1
...
std::vector<T> WindowFunctions<T>::createTukeyWindow (int numSamples, T alpha)
{
 ...
 minus 1
    ...
    for (int i = 0; i < numSamples; i++)        // left taper
    {
        ...
        {
            window[i] = 0.5 * (1 + cos (M_PI * ((( 2 * value) / (alpha * numSamp
lesMinus1)) - 1)));
        }
