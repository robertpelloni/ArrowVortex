...
template <typename T>
inline bool Cmp(const SegmentRow<T>& segment, Row row) {
    return segment.row < row;
}

template <typename T>
inline bool Cmp2(Row row, const SegmentRow<T>& segment) {
...
template <typename T>
SegmentChanges<T> PersistentSegmentTrack<T>::modify(
    const SegmentModification<T>& modification) {
    ...
    for (auto& [targetRow, targetValue] : modification) {
        ...
        if (i < mySegments.size() && mySegments[i].row == targetRow) {
            ...
            if (currentValue != targetValue) {
                ...
                mySegments.insert(mySegments.begin() + i,
                                  {targetRow, targetValue});
            }
        }
    }
    ...
    if (!result.empty()) EventSystem::publish<Tempo::SegmentsChanged>();
...
void PersistentSegmentTrack<T>::reapply(const SegmentChanges<T>& result) {
    ...
    for (auto& [row, change] : result) mod[row] = change.after;
...
void PersistentSegmentTrack<T>::undo(const SegmentChanges<T>& result) {
    ...
    for (auto& [row, change] : result) mod[row] = change.before;
...
template <typename T>
SegmentChanges<T> IsolatedSegmentTrack<T>::modify(
    const SegmentModification<T>& modification) {
    SegmentChanges<T> result;
...
#define DEF_PERSISTENT(T)                                      \
    ...
    template SegmentChanges<T> T::Track::modify(               \
        const SegmentModification<T>&);                        \
