...
class TimingData {
   public:
    struct Event {
        Event(Row row, double startTime, double hitTime, double endTime,
              double spr)
            : row(row),
              startTime(startTime),
              hitTime(hitTime),
    ...
    struct TimeTracker {
        ...
        // Advances the current row, and returns the startTime corresponding to
        // that row.
        double advance(Row row);

        // Looks at a future row and returns the startTime corresponding to that
        // row.
    ...
    struct RowTracker {
        ...
        // Advances the current startTime, and returns the row corresponding to
        // that startTime.
        double advance(double time);

        // Looks at a future startTime and returns the row corresponding to that
        // startTime.
    ...
    // Returns a representation of stops/BPM changes optimized for timing
    // calculations.
    const vector<Event>& events() const;

    // Returns a representation of startTime signatures optimized for timing
    // calculations.
