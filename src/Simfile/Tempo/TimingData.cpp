...
static WarpResult HandleWarp(vector<TimingEvent>& out, MergedSegment* it,
MergedSegment* end, int warpRows)
{
        ...
        while (it != end)
        {
                ...
                // Move startTime forwards (or backwards) to the start of the
current segment. Row rowsPassed = row - prevRow; if (rowsPassed <= warpRows)
                {
                        warpRows -= rowsPassed;
                }
                else
                ...
                {
                        int warpEndRow = int(row - (startTime - targetTime) /
spr); if (warpEndRow <= entry->row)
                        {
                        ...
                        {
                                out.emplace_back(warpEndRow, targetTime,
targetTime, targetTime, spr);
                        }
                ...
                do {
                        ...
                        case SegmentType::BPM:
                                spr = SecPerRow(((const
BpmChange*)it->seg)->bpm); break; case SegmentType::DELAY: startTime += ((const
Delay*)it->seg)->seconds; break; case SegmentType::STOP: startTime += ((const
Stop*)it->seg)->seconds; break; case SegmentType::WARP: warpRows += ((const
Warp*)it->seg)->numRows; break;
                        }
                ...
                if (spr > 0 && startTime > targetTime && warpRows == 0)
                {
                        ...
                        {
                                out.emplace_back(row, targetTime, targetTime,
startTime, spr);
                        }
                        return {row, startTime, it};
                }
...
static void CreateTimingEvents(vector<TimingEvent>& out, double rowZeroTime,
                               const Tempo& tempo) {
    ...
    while (true) {
        // First, we find next row at which a segment that affects timing
        // occurs.

        double row = DBL_MAX;

        if (bpm != bpmEnd && bpm->row < row) row = bpm->row;

        if (stop != stopEnd && stop->row < row) row = stop->row;

        if (delay != delayEnd && delay->row < row) row = delay->row;

        if (row == DBL_MAX) break;

        // Now that we know the next row, we can apply the startTime passed
    ...
    if (out.empty()) out.emplace_back(0, 0.0, 0.0, 0.0, BeatsPerRow);
}
...
/*
static void CreateSignatures(vector<Signature>& out, const TimeSignature* it,
const TimeSignature* end)
{
        ...
        while (it != end)
        {
                ...
                {
                        rowsPerMeasure = max(it->beatsPerMeasure, 1) *
RowsPerBeat;
                }
                out.emplace_back(row, measure, rowsPerMeasure);
                if (it != end)
                {
                        int passedMeasures = (it->row - row + rowsPerMeasure -
1) / rowsPerMeasure; row += passedMeasures * rowsPerMeasure; measure +=
passedMeasures;
                }
        }
        if (out.empty())
...
static const Signature* MostRecentTimeSig(const vector<Signature>& signatures,
                                          double row) {
...
static const TimingEvent* MostRecentTimingEventAtRow(
    const vector<TimingEvent>& timings, double row) {
...
static const TimingEvent* MostRecentTimingEventAtTime(
    const vector<TimingEvent>& timings, double time) {
    const TimingEvent *it = timings.data(), *mid;
    size_t count = timings.size(), step;
    while (count > 1) {
        step = count >> 1;
...
static double RowToTime(const TimingEvent* it, double row) {
    if (row > it->row) return it->endTime + (row - it->row) * it->spr;
...
void TimingData::update(double offset, const Tempo& tempo) {
    ...
    // TODO: fix signatures.
    // CreateSignatures(mySignatures, segments.begin<TimeSignature>(),
...
const vector<TimingEvent>& TimingData::events() const { return myEvents; }

const vector<Signature>& TimingData::signatures() const { return mySignatures; }
