...
void DialogThemeEditor::addColor(const char* label, colorf& color) {
    ...
    // Let's add a "Refresh" button or just rely on the fact that some things
    // might not update instantly. Actually, GuiDraw textures are static. We
    // MUST call GuiDraw::create() (or a new refresh function) to see changes.
    ...
    cp->onChange.bind([]() {
        ...
        // Let's try just re-running the texture generation functions if we can
        // expose them. For now, full destroy/create is the safest "brute force"
        // way, assuming no one holds raw pointers to GD internals.
        GuiDraw::destroy();
        GuiDraw::create();
    });
}
...
void DialogThemeEditor::onSave() {
    String path;
    int index = 0;
    path = gSystem->saveFileDlg("Save Theme", "theme.xml",
                                "XML Files (*.xml)|*.xml", &index);
    if (path.len() > 0) {
        Theme::save(path);
        GuiDraw::destroy();
        GuiDraw::create();
