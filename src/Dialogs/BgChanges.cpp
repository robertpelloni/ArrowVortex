...
void DialogBgChanges::myUpdateWidgets() {
    ...
    if (myCurrentLayer == 2)
        myChanges = gMetadata->getFgChanges();
    else
        myChanges = gMetadata->getBgChanges(myCurrentLayer);
...
void DialogBgChanges::myRefreshList() {
    ...
    for (const auto& c : myChanges) {
        String s =
            Str::fmt("[%1] %2").arg(Str::val(c.startBeat, 3)).arg(c.file);
...
void DialogBgChanges::onAdd() {
    ...
    std::sort(myChanges.begin(), myChanges.end(),
              [](const BgChange& a, const BgChange& b) {
                  return a.startBeat < b.startBeat;
              });

    // Update backend
    if (myCurrentLayer == 2)
        gMetadata->setFgChanges(myChanges);
    else
        gMetadata->setBgChanges(myChanges, myCurrentLayer);
...
void DialogBgChanges::onFileChange(String& val) {
    if (mySelectedIndex >= 0 && mySelectedIndex < myChanges.size()) {
        ...
        myChangeList->setItemText(
            mySelectedIndex,
            Str::fmt("[%1] %2")
                .arg(Str::val(myChanges[mySelectedIndex].startBeat, 3))
                .arg(val));
    }
}
...
void DialogBgChanges::onBrowseFile() {
    ...
    String path =
        gMetadata->findBackgroundFile();  // Reuse existing helper for now, or
                                          // implement generic browse
    if (path.len()) {
