...
void DialogChartStatistics::myUpdateWidgets() {
        ...
        while (it != chart->notes.end()) {
            ...
            while (it != chart->notes.end() && it->row == row) {
                if (it->type == NoteType::NOTE_MINE) {
                    ...
                    if (it->type == NoteType::NOTE_STEP_OR_HOLD ||
                        it->type == NoteType::NOTE_HOLD_HEAD ||
                        it->type == NoteType::NOTE_ROLL_HEAD ||
                        it->type == NoteType::NOTE_LIFT ||
                        it->type == NoteType::NOTE_FAKE) {
                        notesOnRow++;
                        steps++;
                    }
                }
    ...
    if (!chart->notes.empty()) {
        ...
        myMaxDensity = 0.0f;
        for (float v : myDensityData)
            if (v > myMaxDensity) myMaxDensity = v;
