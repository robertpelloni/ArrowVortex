...
void DialogGoTo::myCreateWidgets() {
    ...
    myInput->setTooltip(
        "Enter time (mm:ss.ms), beat (b123), measure (m10), or row number.");
...
void DialogGoTo::onGo() {
    ...
    if (Str::startsWith(s, "b")) {
        ...
        // Measure (assume 4 beats per measure if not standard? Or use TempoMan
        // logic if available?) TempoMan beatToMeasure exists. We need
        // measureToBeat/Time. Standard: Measure = Beat / 4.0? Or variable time
        // sig? TempoMan handles variable time sigs usually. But I don't see
        // `measureToBeat` in `TempoMan`. Let's assume standard 4/4 for simple
        // input or just Beat*4.
        String val = s.subStr(1);
        double measure = Str::toDouble(val);
        double beat = measure * 4.0;  // Approximation if no time sig support in
                                      // TempoMan reverse lookup
        targetTime = gTempo->beatToTime(beat);
