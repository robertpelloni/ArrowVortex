...
template <typename T>
static WgLineEdit* CreateField(RowLayout& layout, StringRef label, String& str,
                               T func) {
...
void DialogSongProperties::myCreateWidgets() {
    ...
    w = CreateField(myLayout, "Title", myTitle,
                    [](String& s) { gMetadata->setTitle(s); });
    w->setTooltip("Title of the song");

    w = CreateField(myLayout, "Subtitle", mySubtitle,
                    [](String& s) { gMetadata->setSubtitle(s); });
    w->setTooltip("Subtitle of the song");

    w = CreateField(myLayout, "Artist", myArtist,
                    [](String& s) { gMetadata->setArtist(s); });
    w->setTooltip("Artist of the song");

    w = CreateField(myLayout, "Credit", myCredit,
                    [](String& s) { gMetadata->setCredit(s); });
    w->setTooltip("Author of the simfile");

    auto selectable = myLayout.add<WgCheckBox>("Selectable");
    selectable->value.bind(&myIsSelectable);
    selectable->onChange.bind<void, bool>(
        [](bool b) { gMetadata->setSelectable(b); },
        true);  // Placeholder bind, manual update in myUpdateProperties?
    // WgCheckBox binds to bool pointer. We need to bridge to
    // gMetadata->setSelectable. Since widgets update the pointer, we can use a
    // lambda that reads the pointer. But `onChange` is called when clicked.
    ...
    w = CreateField(myLayout, "Music", myMusic,
                    [](String& s) { gMetadata->setMusicPath(s); });
    ...
    w = CreateField(myLayout, "BG", myBackground,
                    [](String& s) { gMetadata->setBackgroundPath(s); });
    w->setTooltip(
    ...
    w = CreateField(myLayout, "Banner", myBanner,
                    [](String& s) { gMetadata->setBannerPath(s); });
    w->setTooltip(
    ...
    w = CreateField(myLayout, "CD Title", myCdTitle,
                    [](String& s) { gMetadata->setCdTitlePath(s); });
    w->setTooltip(
