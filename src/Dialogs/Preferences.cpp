...
void DialogPreferences::myCreateWidgets() {
    ...
    auto cbMiddle =
        myLayout.add<WgCheckbox>("Middle Mouse Button to insert beat");
    ...
    auto cbScroll = myLayout.add<WgCheckbox>(
        "Enable scrolling cursor effect during playback");
    ...
    auto cbInsert =
        myLayout.add<WgCheckbox>("Inserting the same arrow will delete it");
    ...
    auto cbPaste = myLayout.add<WgCheckbox>(
    ...
    auto cbAssist =
        myLayout.add<WgCheckbox>("Assist Tick on beats and sub-beats");
    ...
    cbPractice->onChange.bind(this,
                              &DialogPreferences::onPracticeEnabledChanged);
    cbPractice->setTooltip(
        "Uncheck: AutoPlay (Inputs handled by Editor logic)");

    // Timing windows (Milliseconds)
    // UI layout: Label | Input (Spinner) | Unit

    auto addRow = [&](const char* label, double* val,
                      void (DialogPreferences::*cb)()) {
    ...
    addRow("Marvelous: +/-", &myWindowMarvelous,
           &DialogPreferences::onWindowMarvelousChanged);
    addRow("Perfect: +/-", &myWindowPerfect,
           &DialogPreferences::onWindowPerfectChanged);
    addRow("Great: +/-", &myWindowGreat,
           &DialogPreferences::onWindowGreatChanged);
    ...
    addRow("Freeze OK: +/-", &myWindowFreeze,
           &DialogPreferences::onWindowFreezeChanged);
    ...
    slGain->setRange(1.0,
                     10000.0);  // Log scale would be better but linear for now
...
// Callbacks - Use bound member variables (which are updated by widget before
// callback)
void DialogPreferences::onNudgeChanged() {
    gEditor->setNudgeBasedOnZoom(myNudgeBasedOnZoom);
}
void DialogPreferences::onAssistTickChanged() {
    gEditor->setAssistTickBeats(myAssistTickBeats);
}
void DialogPreferences::onDedupChanged() {
    gEditor->setRemoveDuplicateBPMs(myRemoveDuplicateBPMs);
}
void DialogPreferences::onBeatVolChanged() {
    gEditor->setBeatAssistVol(myBeatAssistVol);
}
void DialogPreferences::onNoteVolChanged() {
    gEditor->setNoteAssistVol(myNoteAssistVol);
}

void DialogPreferences::onMiddleMouseChanged() {
    gEditor->setMiddleMouseInsertBeat(myMiddleMouseInsertBeat);
}
void DialogPreferences::onScrollCursorChanged() {
    gEditor->setScrollCursorEffect(myScrollCursorEffect);
}
void DialogPreferences::onInsertSameChanged() {
    gEditor->setInsertSameDeletes(myInsertSameDeletes);
}
void DialogPreferences::onEditOneLayerChanged() {
    gEditor->setEditOneLayer(myEditOneLayer);
}
void DialogPreferences::onPasteOverwritesChanged() {
    gEditor->setPasteOverwrites(myPasteOverwrites);
}
void DialogPreferences::onSelectPastedChanged() {
    gEditor->setSelectPasted(mySelectPasted);
}
void DialogPreferences::onBackupSavesChanged() {
    gEditor->setBackupSaves(myBackupSaves);
}
void DialogPreferences::onDontShowFPSChanged() {
    gEditor->setDontShowFPS(myDontShowFPS);
}
void DialogPreferences::onPythonPathChanged() {
    gEditor->setPythonPath(myPythonPath);
}

void DialogPreferences::onPracticeEnabledChanged() {
    gEditor->setPracticeMode(myEnablePracticeMode);
}
...
void DialogPreferences::onWindowMarvelousChanged() {
    UPDATE_SETUP(windowMarvelous, myWindowMarvelous);
}
void DialogPreferences::onWindowPerfectChanged() {
    UPDATE_SETUP(windowPerfect, myWindowPerfect);
}
void DialogPreferences::onWindowGreatChanged() {
    UPDATE_SETUP(windowGreat, myWindowGreat);
}
void DialogPreferences::onWindowGoodChanged() {
    UPDATE_SETUP(windowGood, myWindowGood);
}
void DialogPreferences::onWindowBooChanged() {
    UPDATE_SETUP(windowBoo, myWindowBoo);
}
void DialogPreferences::onWindowFreezeChanged() {
    UPDATE_SETUP(windowFreeze, myWindowFreeze);
}
void DialogPreferences::onWindowMineChanged() {
    UPDATE_SETUP(windowMine, myWindowMine);
}

void DialogPreferences::onWaveformColorModeChanged() {
    if (gWaveform)
        gWaveform->setColorMode((Waveform::ColorMode)myWaveformColorMode);
}
void DialogPreferences::onAntiAliasingChanged() {
    if (gWaveform) gWaveform->setAntiAliasing(myAntiAliasing);
}
void DialogPreferences::onShowOnsetsChanged() {
    if (gWaveform) gWaveform->setShowOnsets(myShowOnsets);
}
void DialogPreferences::onOnsetThresholdChanged() {
    if (gWaveform) gWaveform->setOnsetThreshold((float)myOnsetThreshold);
}
void DialogPreferences::onSpectrogramGainChanged() {
    if (gWaveform) gWaveform->setSpectrogramGain((float)mySpectrogramGain);
}
void DialogPreferences::onRGBCrossoverChanged() {
    if (gWaveform)
        gWaveform->setRGBCrossovers((float)myRGBLow, (float)myRGBHigh);
}
